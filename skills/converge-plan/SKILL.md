...
用户同时授权“先写计划，再逐步修复”时，计划完成后切换到执行 Skill，并遵循 [分步可见交付](../../references/execution-control.md#分步可见交付)：执行前核对原生计划工具，同步成功或明确说明文字降级。Plan 文件校验通过不代表宿主面板已创建，不能只给计划链接或用文字汇报冒充原生分步显示。
