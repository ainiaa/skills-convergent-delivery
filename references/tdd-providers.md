...
Evidence v2 的可选 `test_check={executed}` 只由本次 stdout/stderr 中 runner 的执行汇总生成，并加入指纹。unittest 扣除 skipped；pytest 计 passed/failed；Maven 使用最后一个 Surefire 汇总并扣除 skipped；Jest/Vitest 使用 Tests 汇总；Gradle 需要项目本次输出 `N tests completed[, M failed][, K skipped]`。静默、定制且无法解析的输出保持 uncovered，不读取历史报告来补证据。此计数证明执行非空，不证明断言质量；selector 仍须绑定对应 runner 的真实选择参数，pytest 的输出路径选项值不能冒充测试路径。unittest 只允许一个点分目标；pytest 文件模式只允许该单个位置目标，或使用唯一 -k/--keyword 表达式过滤实际执行。未知 pytest 选项不猜测其参数含义。Maven/Gradle/Jest/Vitest 拒绝重复测试选择参数，防止无关测试替空目标提供计数。
