...
- `coverage`：`{"status":"covered","threshold":1..100,"receipt":<final passing observed receipt>}`，或 `{"status":"uncovered","reason":<non-empty>}`。后者使 trace 不能 native complete。Evidence Receipt 和 trace 均有有界 argv、字符串与总大小；命令行不得携带 token、password、secret 或 key，改用进程环境或项目的受控凭据配置。回执防止误写和事后不一致，但不提供同一工作区用户对抗篡改的密码学证明。
