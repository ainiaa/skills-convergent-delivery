...
| 跨会话 checkpoint | Plan v5 为 `checkpoint=cross_session` | 进入 Batch 前单独请求本地 commit 授权；可信本地当前会话 binding 不自动恢复 worker，仍需 `host_observed` bridge；授权不包含 push、tag、merge 或发布。 |
