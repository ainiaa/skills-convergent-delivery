...
所有运行时变更还须通过 [TDD/Impact Trace](tdd-providers.md#tddimpact-trace-v3)：它以一条改动入口和已知关联链绑定 observed 红绿 Evidence Receipt 与最终回归测试，不建立第二状态机。只有明确要求全量收口时，才升级为下述 Plan v6 矩阵。
