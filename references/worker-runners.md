...
- `codex-exec-v1` 与 `claude-code-v1` 都是本地进程 runner，分别只接受 OpenAI 与 Anthropic 的 effective provider，且 requested/effective 的 model 与 effort 必须完全相同。计划冻结 CLI 绝对路径及内容指纹，执行前再次验证；prompt 仅经标准输入传递，不进入命令参数。Codex 读取用户的 `$CODEX_HOME/config.toml`，而冻结的 `--model`、reasoning effort 与 sandbox 参数优先，保证 leaf runner 沿用用户已配置的能力但不能扩大本次权限；Claude 显式传入 `--bare --strict-mcp-config --model --effort --max-turns`、冻结工具与 permission mode。Claude 这是一层 CLI permission 边界，不是 OS sandbox。
...
reviewer 执行某个冻结 Review v3 请求时，controller 必须在 lifecycle 的单 launch `--review-request` 或 fan-out `--review-requests` 中传入完整 canonical request。lifecycle 使用同一 `review_contract.py` 归一化并重算 SHA-256，且要求 task、baseline commit 与 source fingerprint 均匹配当前 managed run；可选的 `--review-request-fingerprint(s)` 只作为相等性断言。三个 runner 将派生出的摘要作为仅用于 ledger 绑定的冻结 configuration 字段保存，执行命令和 HTTPS body 均不消费它；非 reviewer、缺 request 或不匹配值确定性拒绝。完成门禁只接受与同一 reviewer identity、同一 request fingerprint 绑定的可用 role result；identity 可为已完成的 host worker，或为外部 runner 的冻结 `profile.worker_id`，后者绝不声称为 host-native worker。
