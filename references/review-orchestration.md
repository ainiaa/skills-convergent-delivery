...
控制器按风险选择复核成本。需求符合性与实现质量仍分别保存结论，但普通任务由一个 fresh reviewer 在一次请求中返回两轴；低风险任务使用实现者自检和新鲜验证，不创建 reviewer。高风险任务使用一个 blind reviewer。只有多任务或跨服务计划才增加一次 integration review。任何源码变化都会使旧结果 stale。
