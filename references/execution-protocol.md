...
3. 用 `evidence_contract.py run` 运行定向验证，保留 observed Evidence Receipt；每个绿灯定向重跑一次作为稳定性 rerun，并在最终源码上运行 CodeGraph；未索引或不可用时写入 graph `uncovered`。先以 `native_tdd_policy.py resolve --workspace <workspace>` 解析 coverage：pytest/Vitest 可注入 `quality-targets.yml` 或默认 >=85%，其他命令须已有可证明 gate。高风险补一条 integration/contract mutation 检查。无法确认或没有可执行工具时标为 `uncovered`。按 [TDD 追溯](tdd-providers.md#tddimpact-trace-v4) 校验测试、风险与影响链回执。
