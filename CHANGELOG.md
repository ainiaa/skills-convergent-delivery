...
- 修复显式 Codex autonomy Hook 的同 task 续跑：Desktop `session_id` 与 CLI `thread_id` 统一经 `codex queue --thread` 投递冻结下一动作；先持久化一次性 intent，重复 Stop、缺标识或投递失败均安全终止并释放 lease，不创建 successor task。
