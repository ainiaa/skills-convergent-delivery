...
- 精简普通 Plan v6：可省略全量收口专属的 `closure_matrix` 和规划图回执，完成审计仍核对任务范围、源码连续性及最终验收；显式全量收口仍强制矩阵与图回执，已有带矩阵计划继续完整校验。原生 TDD 的重复绿灯次数保持不变，避免以减少验证代替等价精简。
- 将“新需求/Bug 修复 → 冻结范围与实现逻辑 → 同任务实现、复核、修复 → 有证据结束”写入 Converge 核心契约；完成状态拒绝任何已记录待处理问题，报告不再把此类状态包装为“已完成，需关注”。新增单轮双回归交互场景和完成门禁回归；真实宿主模型行为仍需独立 smoke 验证。
- GPT-6 适配：根 Skill 将仅供逐步执行的长协议改为按需加载；多模型配置只生成可派发角色画像，兼容旧六角色 v4 配置并明确拒绝串行角色覆盖；仓库任务评测可比较同模式下不同模型/推理档位。复核 1+1 预算仍限定单次 run，不撤销同会话对不同同范围根因的新有限修复授权；真实模型质量与成本未评测时保持未覆盖。
- 同会话同事项的已授权修复在完成报告后仍可对新发现的同范围 Bug 开启有限新轮次；新会话只读检查需另行确认修复，明确修复请求不二次确认。Codex repair 快捷指令补齐“继续修复已知问题”，新建 Hook 门禁要求宿主 session ID 并绑定活动 Stop，其他会话不能接管；完成记录仅为后续检查提供范围线索，不凭相同仓库自动重开旧任务。
- 当 Hook 继承外层仓库的 `GIT_DIR`/`GIT_WORK_TREE`、但 `cwd` 属于内层独立仓库时，Stop 仅按内层归属检查受管状态：内层活动运行被拦截，即使外层活动而内层无状态也不会误拦。显式“继续修复”拒绝误以外层仓库启动门禁；显式工作树根目录已有不同 `.git` 标记时同样适用。
- Stop Hook 遇到指向其他工作树的 `GIT_DIR`/`GIT_WORK_TREE` 时，仍检查当前 `cwd` 的本地及 Git common-dir 活动状态，不再提前误放行；无受管状态的工作区仍放行。显式“继续修复”入口拒绝把门禁建到与请求 `cwd` 无归属关系的工作树。
- Stop Hook 现能在 `GIT_DIR`/`GIT_WORK_TREE` 指定的合法工作树中优先使用显式 Git 身份，即使父目录另有 `.git` 也从真实 common-dir 找到活动状态，避免误放行；Git 环境无效时结构化阻断，无受管状态时正常放行。Codex 写入 runner 保留 `rev-parse` 路径末尾空格，合法隔离 worktree 不再被误拒。
- Stop Hook、显式“继续修复”入口及写租约身份核验把悬空 `.git` 符号链接识别为无法确认的 Git 布局，不再误放行、静默跳过修复门禁或绕过租约验证；读取 Git 根路径、common-dir 和 `.git` 指针时只去除输出换行，保留目录名末尾的合法空格，避免 Stop 漏掉活动状态、租约身份分裂或把新状态写进错误目录。
- Stop Hook 识别其他 linked worktree 时额外核对管理目录位于 Git common-dir 的 `worktrees/<id>` 下，并拒绝符号链接形式的 `.git` 标记；仓库外自建双向指针或链接到合法 worktree 的伪装目录不再能隐藏共享目录中的活动状态。
- 放宽派发成功路径测试专用的进程启动期限，并在读取子进程产物前先断言派发结果，避免覆盖率采集下的启动抖动掩盖真实失败；生产派发超时策略未变。
- 共享 Git 状态仅在 common-dir 与 worktree 管理目录的反向指针均证实归属时跳过；归属路径缺失、属于独立仓库或复制 `.git` 指针伪装的活动记录不再让 Stop Hook 误放行。扫描旧租约时，无关工作区的损坏 Git 指针不再阻断当前获取；可能属于当前工作树的损坏指针仍安全阻断。
- Stop Hook 在确认当前工作树没有活动自治状态时不再因 Git 查询超时拦截普通任务；有活动状态而 Git 查询超时、失败或布局不可确认时仍安全阻断。未知 schema、缺失或错配 workspace 的当前活动状态返回结构化错误；共享状态目录中明确属于其他 worktree 的记录不误拦。精确“继续修复”在 Git 查询失败时不再静默跳过 gate。
- 修复升级前 Controller Snapshot 在子目录 Stop 时误放行，以及同一 Git 工作树根目录/子目录可分别取得写租约的问题；旧租约释放竞态与受管 Git 路径查询失败时均拒绝绕过互斥，保留不同 worktree 的并行能力。
- 用户级多模型 profile 改为显式选择才读取，普通运行使用内置默认；修正本机 Claude profile 的 implementer 为受限 Codex runner。
- 更新多模型内置默认至 GPT-6 Luna/Sol，并将串行 adjudicator 的 Astra 画像调为 medium；旧版显式 profile 保持可用。
- 将 Codex `continue repair` 快捷入口的未知任务画像改为高不确定性，避免在缺少已冻结低风险画像时降级为低审查的 inline 路由。
- `continue repair` 统一以 Git 根目录建立和查找 Hook 状态；路径风险只从真实变更派生，未改动的 `SECURITY.md` 不再误升高审查等级，子目录触发的 Stop 也能接续同一 run；非 Git cwd 不再阻断普通任务。
