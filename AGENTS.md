...
- 本仓全量生产 Python 覆盖率必须达到 90%；不得通过缩小统计范围、排除核心路径或降低门槛取得通过。

## 代码导航与读取纪律

- 代码定位、调用链和影响面先用 CodeGraph；图谱命中后，只读取完成当前判断或编辑所需的最小源码片段。
- 同一文件的同一未变区域不得重复读取。保留并复用已有图谱和读取结果；文件被编辑、图谱内容截断，或必须核对当前补丁上下文时，才允许重新读取。
- CodeGraph 不替代逐字核实：编辑前后可直接读取当前文件的相关片段，但不得把“核实”扩展为重复读取整份文件。
...
| 自进化研究（暂不进入运行时） | [自进化参考备忘](docs/02_design/architecture/self-improving.md)、Darwin Skill、Microsoft SkillOpt、gstack domain skills |
