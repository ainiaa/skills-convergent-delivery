...
当前开发版本：[0.13.0](VERSION)。尚未创建 Git tag 的改动记录在 [Unreleased](CHANGELOG.md) 中。
...
- 复杂任务先形成 Plan Contract v3；每个 task 只有一个结果、明确范围、依赖和真实验证。
...
- 实现循环、单任务双轴审查循环和全局集成审查循环各有独立终止条件；重复问题、无进展或预算用尽时停止。
