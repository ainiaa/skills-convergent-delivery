...
TDD 覆盖：涉及运行时行为的功能或修复，先将每条验收项至少映射一个测试，再改生产代码；测试至少覆盖正常、边界和异常场景，权限、并发或幂等风险按任务语义加入，金额或支付还须包含 property 场景，时间、时区和不可逆操作分别加入 time、timezone、recovery integration 场景。测试优先覆盖公共 seam，每个测试只验证一个可观察行为；mock 只用于外部系统边界，不验证内部调用顺序。先按实际调用链列出入口、调用方、共享副作用或外部契约等影响链，并把每条链绑定到本次回归测试。红灯与绿灯回执必须是由 `evidence_contract.py run` 产生的 observed Evidence Receipt；每个测试须给出出现在红绿 `argv` 中的精确 `selector`，pytest、Gradle、Maven、Vitest/Jest 等已知 runner 必须使用其 runner selector 语法。红灯还必须声明 `failure_class` 为 `missing_behavior|assertion`；编译、Mock 或环境错误不能当作有效红灯。所有有风险任务的绿灯须有两次稳定性 rerun；可用时 CodeGraph receipt 必须执行由影响链确定生成的查询，不可用时图谱范围保持 `uncovered`；金额、支付、权限、安全、事务、并发、幂等、数据访问和契约风险还须有 mutation 检查的 integration/contract 测试。原生 `native-v1` 的 coverage 项目配置优先：先用 `python3 "$CONVERGE_SKILL_DIR/scripts/native_tdd_policy.py" resolve --workspace <workspace>` 解析，`test-commands.yml` 的安全 coverage argv 优先；pytest 和 Vitest 会注入 `quality-targets.yml` 或默认 >=85% 的阈值，Maven/Gradle 仅在已运行 JaCoCo gate 且项目阈值足够时放行，Rust `--fail-under` 和 .NET `/p:Threshold` 等 argv 内显式 gate 也可识别，其余命令保持 `uncovered`。没有可执行 coverage 工具时保持 `uncovered`。最终将验收、测试标识、selector、场景、红绿回执、冻结 `risk_flags`、coverage、mutation 与影响链按 [TDD 追溯](references/tdd-providers.md#tddimpact-trace-v5) 输入 `python3 "$CONVERGE_SKILL_DIR/scripts/tdd_impact_guard.py" validate --input -`；native-v1 最终验收前以 `rerun --input - --workspace <workspace> --baseline <commit> --native-coverage` 实际重跑 trace 的最终绿灯、coverage、mutation 与图谱命令，其中 coverage 回执的 argv 和阈值必须精确匹配该项目的 resolve 结果，并将返回 trace 写回 state。PDLC 保持自己的 coverage 门槛，rerun 不套用 native coverage 策略。native-v1 只有 `ledger.tdd_trace` 返回 `pass`、匹配最终源码、冻结风险和当前全部验收项时才能进入 `complete`；失败或无法证明的验收范围保持 `uncovered`，不得声称关联功能未受影响。
