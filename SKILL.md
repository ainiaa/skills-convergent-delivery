...
闭环v11；终态只认新鲜证据。闭环先 arm 当前 workspace 唯一 active run；active run 未到 `complete|blocked` 时不得输出 final。

## 开始

Skill 根目录记为 `CONVERGE_SKILL_DIR`。冻结验收/路径/基线，仅改 task diff；不可逆问用户。模型自述不放行，`unknown`失败，走完整路径。
...
`planned_task=true` 仅执行 capsule；否则按 [任务路由](references/task-routing.md) 用 `task_profile.py [--full-closure]` 分类，持久状态用 `freeze_routing` 冻结画像与请求摘要。全量收口控制器明确决定，禁止关键词猜测；`full_closure_required=true` 禁止 `inline`，先经 `converge-plan`；`autonomy_begin --full-closure`一律拒绝。仅当路由不是 `inline` 读 [计划执行与无响应保护](references/execution-control.md)。

复杂/未知/长任务用 `converge-plan`，按独立可验收的业务切片执行；仅 `cross_session` 进 `converge-batch` 并先获 commit 授权。全量收口需矩阵和当前源码 gate；缺项标 `uncovered`，不得宣称全部完成；见 [Plan Contract](skills/converge-plan/references/plan-contract.md)。
